...
md.renderer.rules.link_open = (tokens, idx, options, env, self) => {
  ...
  if (href.indexOf('not-allowed-as-link') >= 0) {
    ...
    tokens[idx].attrs[0][1] = config.toUrl(decodeURIComponent(href)) || href
  }
...
module.exports = (input, opts) => {
  ...
  // Keep track of processed links so we don't process them twice.
  //
  // When a link comes through we cache the result, so if the same link comes
  // through again we can just `return` it immediately rather redoing it.
  const toUrl = config.toUrl
  const processedLinks = []
  config.toUrl = (ref) => {
    if (processedLinks.includes(ref)) {
      return ref
    } else {
      const result = toUrl(ref)
      processedLinks.push(result)
      return result
    }
  }
